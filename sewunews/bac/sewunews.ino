...
// Clock Display State (cycle: 0=Time, 1=Day, 2=Date)
int clockMode = 0;
...
void loop() {
  ...
  // Cycle clock display every 3 seconds
  if(millis() - lastClockCycle > 3000) {
    lastClockCycle = millis();
    clockMode = (clockMode + 1) % 3; // 0=Time, 1=Day, 2=Date
  }
  ...
  if (millis() - lastScroll > cfg.scrollSpeed) {
    ...
    if(clockMode == 0) {
      ...
      // DAY: SABTU
      topText = String(getHari());
    } else {
      // DATE: 25 DEC 2025
      char buf[20];
      sprintf(buf, "%02d %s %04d", tgl, getBulan(), tahun);
      topText = String(buf);
    }
    
    int wTop = Disp.textWidth(topText.c_str());
    int xTop = (64 - wTop) / 2; // Centered
    if(xTop < 0) xTop = 0;
