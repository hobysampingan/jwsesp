...
select,input[type="text"],input[type="password"]{width:100%;padding:12px;font-size:15px;border:2px solid #e0e0e0;border-radius:10px;background:#fff;transition:all 0.3s;margin-bottom:10px}
select:focus,input:focus{outline:none;border-color:#667eea}
.slider-container{display:flex;align-items:center;gap:15px}
.slider{flex:1;height:8px;border-radius:5px;background:#e0e0e0;outline:none;-webkit-appearance:none;appearance:none}
...
.speed-value{min-width:60px;padding:8px 12px;background:#f0f0f0;border-radius:8px;text-align:center;font-weight:bold;color:#667eea}
.btn{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;border:none;padding:15px 30px;font-size:16px;border-radius:10px;cursor:pointer;width:100%;margin-top:10px;font-weight:bold;box-shadow:0 4px 15px rgba(102,126,234,0.4);transition:all 0.3s}
...
.grid-col-2{display:grid;grid-template-columns:1fr 1fr;gap:10px}
</style>
</head>
<body>
<div class="container">
<div class="header">
<h1>🗞️ SEWU NEWS</h1>
<p>Konfigurasi Sumber Berita & Pengaturan</p>
</div>
<div class="content">
<div class="info-box">
...
<button class="btn" onclick="saveWiFi()">💾 Simpan WiFi & Reboot ESP</button>
<div class="status" id="statusWiFi"></div>
</div>

<div class="section">
<div class="section-title">⚡ Kecepatan Scroll</div>
<div class="slider-container">
<input type="range" min="20" max="100" value="40" class="slider" id="speedSlider" oninput="updateSpeed()">
<div class="speed-value" id="speedValue">40ms</div>
</div>
<small style="color:#666">Geser ke kiri = Cepat, Geser ke kanan = Lambat</small>
</div>
...
<button class="btn" onclick="save()">💾 Simpan Sumber Berita</button>
...
function updateSpeed(){
  ...
  // Auto-save saat geser
  fetch('/api/speed?speed='+speed,{method:'POST'})
  .then(r=>r.json())
  .then(d=>{
...
function saveWiFi(){
  ...
  var pass=$('wifiPassword').value.trim();
  
  if(!ssid){
    showStatus('statusWiFi','SSID tidak boleh kosong!',false);
    return;
  }
  ...
  fetch('/api/wifi',{
    ...
    body:JSON.stringify({wifiSSID:ssid,wifiPassword:pass})
  })
  .then(r=>r.json())
  .then(d=>{
    if(d.ok){
      showStatus('statusWiFi','✅ WiFi disimpan! ESP akan reboot otomatis...',true);
    }else{
      showStatus('statusWiFi','❌ Gagal simpan',false);
    }
  });
}
...
function load(){
  ...
  .then(d=>{
    ...
    $('speedSlider').value=d.scrollSpeed;
    $('speedValue').textContent=d.scrollSpeed+'ms';
...
function save(){
  ...
  .then(d=>{
    if(d.ok){
      showStatus('status','✅ Tersimpan! Berita akan diupdate dalam 10 detik...',true);
      setTimeout(loadNews,11000); // Reload news setelah 11 detik
    }else{
      showStatus('status','❌ Gagal simpan',false);
    }
  });
}
